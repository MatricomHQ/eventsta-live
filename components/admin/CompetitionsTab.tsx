...
import { PlusIcon, MegaphoneIcon, TrashIcon, ArrowLeftIcon, UsersIcon, StopCircleIcon, ShieldIcon, EditIcon } from '../Icons';
    ...
    return (
        <div className="space-y-6 animate-fade-in">
            ...
            <div className="space-y-8">
                ...
                <div className="bg-neutral-900 border border-neutral-800 rounded-2xl p-8">
                    <h3 className="text-xl font-bold text-white mb-6">Competitor Leaderboard</h3>
...
const ApplicantsListView: React.FC<{ competition: Competition, eventId: string }> = ({ competition, eventId }) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                ...
                stats.forEach(entry => statsMap[entry.userId] = entry);
                setLeaderboardData(statsMap);

                // 2. Fetch Users based on the leaderboard or competition list
                // Use competition list to ensure we get everyone even if 0 sales
                if (competition.competitorIds.length > 0) {
                    const users = await api.getUsersByIds(competition.competitorIds);
                    setApplicants(users);
                } else {
                    setApplicants([]);
                }
    ...
    if (loading) return <div className="text-center py-12 text-neutral-500">Loading competitors...</div>;
